...
import { getLessonById, updateLesson } from "@/actions/content"
...
export default function EditLessonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [lessonStreamId, setLessonStreamId] = useState<string | null>(null)
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          setFormData({
            ...
            subject: lesson.category || "Mathematics",
            gradeLevel: lesson.level,
            stream: lesson.stream !== "NONE" ? lesson.stream : "",
            difficulty: "intermediate", // Default or fetch if available
          ...
          // Store module and stream IDs for redirect (for SUPERIEUR lessons)
          if (lesson.level === EducationalLevel.UNIVERSITY) {
            setLessonModuleId(lesson.moduleId || null)
            setLessonStreamId(lesson.educationalStreamId || null)
          }
          // Store slug for PDF download
          setLessonSlug(lesson.slug || null)
        } else {
  ...
  const handleGenerate = async () => {
    ...
    // For UNIVERSITY level, generate chapter content
    if (isUniversity) {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          // Automatically create the chapter after generating content
          if (generatedContent) {
            toast.success("Contenu généré ! Création du chapitre...")
            setIsCreatingChapter(true)
            try {
              // Automatically create the chapter
              ...
              if (createResult.success) {
                ...
                console.log("Chapter created, redirecting...", { lessonStreamId, lessonModuleId, id })

                // Reset chapter fields
                setFormData(prev => ({ ...prev, chapterTitle: "", chapterNumber: 1, content: "", aiInstructions: "" }))

                // Redirect to chapters view
                if (lessonStreamId && lessonModuleId) {
                  setTimeout(() => {
                    const redirectUrl = `/admin/lessons?cycle=SUPERIEUR&streamId=${lessonStreamId}&moduleId=${lessonModuleId}&lessonId=${id}&t=${Date.now()}`
    ...
    // For other levels, generate lesson content
  ...
  const handleCreateChapter = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // For UNIVERSITY level, redirect to admin lessons page showing chapters
        const isUniversity = formData.gradeLevel === "UNIVERSITY" || formData.gradeLevel === EducationalLevel.UNIVERSITY
        if (isUniversity && lessonStreamId && lessonModuleId) {
          // Small delay to ensure database transaction is committed and revalidation completes
          setTimeout(() => {
            const redirectUrl = `/admin/lessons?cycle=SUPERIEUR&streamId=${lessonStreamId}&moduleId=${lessonModuleId}&lessonId=${id}&t=${Date.now()}`
            console.log("Redirecting to:", redirectUrl)
            // Use window.location for a hard refresh to ensure cache is bypassed
            window.location.href = redirectUrl
          }, 300)
        } else {
          // Force refresh of ChapterManager
          setChapterManagerKey(prev => prev + 1)
          router.refresh()
        }
      } else {
        toast.error("Erreur: " + result.error)
        console.error("Chapter creation failed:", result.error)
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    // For UNIVERSITY level, we don't require content (chapters are managed separately)
    ...
    setIsSaving(true)
    try {
      // For UNIVERSITY level, don't update the title or content (it's a container for chapters)
      // The lesson title should remain unchanged - chapters are managed separately
      const result = await updateLesson(id, {
        // For UNIVERSITY, don't update title or content - they are managed via chapters
      ...
      if (result.success) {
        ...
        // For UNIVERSITY level, redirect to admin lessons page showing chapters
        if (isUniversity && lessonStreamId && lessonModuleId) {
          router.push(`/admin/lessons?cycle=SUPERIEUR&streamId=${lessonStreamId}&moduleId=${lessonModuleId}&lessonId=${id}`)
        } else {
          router.push("/admin/lessons")
        }
      } else {
    ...
    <div className="p-6 w-full max-w-[1800px] mx-auto min-h-screen flex flex-col">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        // Pre-fill with chapter example if content is empty
                        if (!formData.content) {
      ...
      {(formData.gradeLevel === "UNIVERSITY" || formData.gradeLevel === EducationalLevel.UNIVERSITY) && (
        <div className="mt-8">
          ...
          <ChapterManager key={chapterManagerKey} lessonId={id} lessonTitle={formData.title} />
        </div>
      )}
    </div>
  )
